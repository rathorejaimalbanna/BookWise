...
function ProductCard(props) {
  ...
  function addItem() { 
    ...
    if (userData.length === 0) {
      ...
      props.addCart(limitTitle(props.item.volumeInfo.title), props.item.volumeInfo.imageLinks?.thumbnail);
  ...
  return (
    <Card className={styles.cardsDiv}>
      ...
      <Card.Img variant="top" src={props.type === "cart"? props.item.image : props.item.volumeInfo.imageLinks?.thumbnail} className={styles.cardImage} alt='product image'/>
      <Card.Body>
        {/* Render product name */}
        <Card.Title className={styles.cardTextTitle}>{props.type === "cart"?props.item.name : limitTitle(props.item.volumeInfo.title)}</Card.Title>
        {/* Render product description (placeholder) */}
        <Card.Text className={styles.cardText}>
          {props.type === "cart" ? 
          <span>Qty: {props.item.qty}</span> : <span>{props.item.volumeInfo.authors[0]}</span>}
        </Card.Text>
        {props.type !== "cart" && (<Card.Text className={styles.cardText}>
